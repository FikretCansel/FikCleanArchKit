...
describe("RegisterUserUseCase", () => {
  it("registers with normalized credentials and publishes a login event", async () => {
    const session = {
    ...
    const result = await new RegisterUserUseCase(authRepository, eventBus).execute({
      username: "  fikret  ",
      password: "  secret  "
    });

    expect(result).toBe(session);
    const credentials = vi.mocked(authRepository.register).mock.calls[0]?.[0];
  ...
  it("does not call the repository when credentials are invalid", async () => {
    ...
    await expect(
      new RegisterUserUseCase(authRepository, eventBus).execute({
        username: "fikret",
        password: "x"
      })
    ).rejects.toThrow("Password en az 3 karakter olmali.");
